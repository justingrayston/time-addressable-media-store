...
async def get_flow_segments(
    ...
    # Sort segments by timerange start
    all_segments.sort(key=lambda s: TimeRange.from_str(s["timerange"]).start)
...
def normalise_and_transfer_media(
    ...
    if "container_mapping" in flow and flow["container_mapping"] is not None:
    ...
    ts_offset_str = segment.get("ts_offset")
    if ts_offset_str is None:
        ts_offset_str = "0:0"
    ts_offset = Timestamp.from_str(ts_offset_str)

    segment_timerange = TimeRange.from_str(segment["timerange"])
    assert (segment_timerange.start is not None)
    assert (segment_timerange.end is not None)

    object_timerange_str = segment.get("object_timerange")
    if object_timerange_str is not None:
        object_timerange = TimeRange.from_str(object_timerange_str)
    else:
        if segment.get("sample_offset") is not None or segment.get("sample_count") is not None:
            raise NotImplementedError(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--token", type=str, default=os.environ.get("TOKEN"),
        help="Bearer token for authentication. Defaults to the 'TOKEN' environment variable"
    )
    ...
    credentials: Credentials
    if args.token:
        from utils.credentials import BearerCredentials
        credentials = BearerCredentials(args.token)
    elif args.oauth2_url and args.client_id and args.client_secret:
        ...
        logger.error(
            "Require either Bearer token (--token), OAuth2 credentials (--oauth2-url, --client-id, --client-secret) "
